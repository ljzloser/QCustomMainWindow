...
void FramelessWidget::showEvent(QShowEvent *event)
{
    ...
    setAttribute(Qt::WA_Mapped);//设置窗体可见
...
{
    //非最大化才能移动和拖动大小
    if (windowState() == Qt::WindowNoState) {//如果是正常模式
        moveEnable = true;
        resizeEnable = true;
    } else {
        moveEnable = false;
    ...
    emit windowStateChange(!moveEnable);//这里传入的是是否可移动,因为最大化的时候是不可移动的

    //解决mac系统上无边框最小化失效的bug
#ifdef Q_OS_MACOS//mac系统
